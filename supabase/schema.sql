...
create table if not exists public.profiles (
  ...
  settings jsonb default '{"notifications_enabled": true, "theme": "system"}'::jsonb
);
...
create table if not exists public.canonical_issues (
  id uuid default gen_random_uuid() primary key,
...
-- Constellation Snapshots (for future 3D view)
create table if not exists public.constellation_snapshots (
  id uuid default gen_random_uuid() primary key,
  user_id uuid references public.profiles(id) on delete cascade not null,
...
-- Aggregate Issues
create table if not exists public.aggregate_issues (
...
create table if not exists public.aggregate_connections (
  id uuid default gen_random_uuid() primary key,
  issue_a_id uuid references public.canonical_issues(id) not null,
  issue_b_id uuid references public.canonical_issues(id) not null,
...
create index if not exists idx_messages_not_refreshed on public.messages(conversation_id) where included_in_refresh = false;
create index if not exists idx_user_issues_user on public.user_issues(user_id);
create index if not exists idx_user_connections_user on public.user_connections(user_id);
create index if not exists idx_aggregate_issues_energy on public.aggregate_issues(energy_score desc);
create index if not exists idx_canonical_issues_name on public.canonical_issues(name);
create index if not exists idx_canonical_issues_active on public.canonical_issues(id) where is_active = true and merged_into_id is null;
